...
// --- Topology module generation ---

function generateTopologyModule(proxyCount: number): string {
	...
	// Deterministic hash of topology-defining fields
	...
	return `// Generated by scripts/deploy.ts — do not edit manually
export const TOPOLOGY = ${JSON.stringify(topology, null, 2)} as const;
...
async function main() {
	...
	for (let i = 1; i <= proxyCount; i++) {
		...
		proxyWorkers.push({ name: `${WORKER_NAME_PREFIX}${String(i).padStart(WORKER_NAME_PAD, "0")}`, configPath, type: "proxy" });
	}
	...
	// Generate topology module
	const topologyDir = path.join(projectRoot, "src", "generated");
	if (!fs.existsSync(topologyDir)) {
		fs.mkdirSync(topologyDir, { recursive: true });
	}
	const topologyModule = generateTopologyModule(proxyCount);
	fs.writeFileSync(path.join(topologyDir, "topology.ts"), topologyModule);
	console.log(`   Topology module: ${path.join("src", "generated", "topology.ts")}`);
